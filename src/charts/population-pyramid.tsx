...
export function PopulationPyramid({
    ...
    const [ legendRef, legendSize ] = useContainerSize<HTMLSpanElement>()
    const { width: legendWidth, height: legendHeight } = legendSize
    ...
    const totalData = data.reduce((acc:totalDataType, curr: ageRangeGroup)=>{
        curr["male"]=+curr["male"]
    ...
    const chartRef = useD3<HTMLDivElement>((container)=>{
        ...
        const margin = { top: 0, right: !isMediumScreen?15:0, bottom: 30, left: !isMediumScreen?15:0 };                        
    ...
    return (
        ...
        >
            { controls}
            <div
                className={`${pyramidStyles.container}`}
            >
                <span 
                    ref={legendRef}
                    className={`${pyramidStyles.inner}`}
